...
```python
f = Filler()
f.set_col_width(col=3, width=30)  # 传入列序号
f.set_col_width(col='c', width=30)  # 传入列号
```
...
```shell
{'row': 2, '姓名': '张三', 'B': '男', '年龄': 20}
{'row': 3, '姓名': '李四', 'B': '女', '年龄': 25}
```
...
|   参数名称   |   类型    | 默认值 | 说明 |
|:--------:|:-------:|:---:|----|
|  `row`   |  `int`<br>`str`  | 必填  | 行号，`'1:5'`格式设置范围 |
| `height` | `float` | 必填  | 行高 |
...
|  参数名称   |       类型       | 默认值 | 说明                           |
|:-------:|:--------------:|:---:|------------------------------|
|  `col`  | `int`<br>`str` | 必填  | 列序号或列号，`'a:d'`或`'1:5'`格式设置范围 |
| `width` |    `float`     | 必填  | 列宽                           |
