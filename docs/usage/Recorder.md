...
只传入一个`CellStyle`对象时整行使用相同格式。

以列表形式传入多个`CellStyle`对象时为每列设置不同格式，不设置格式的列填`None`。

要清除设置的样式则传入`None`。

|   参数名称   |                      类型                      | 默认值 | 说明                |
|:--------:|:--------------------------------------------:|:---:|-------------------|
| `styles` | `CellStyle`<br/>`list`<br/>`tuple`<br/>`None` | 必填  | 单元格格式对象组成的列表或单个对象 |
...
!!! Tip "注意"
    `set.styles()`的优先级比`set.follow_styles()`高。
...
| `add_new` | `bool` | `False` | 数据中有表头不存在的列时是否自动添加到表头，`on_off`为`True`时才有效 |
